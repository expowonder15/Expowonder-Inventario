...
async function cargarDatos() {
  ...
  try {
    ...
    // SUPER URGENTE
    html += `<div class="card">
      <h2 style="color:#C0392B">&#128680; Super urgente — conseguir ya (${superUrg.length})</h2>`;
    if (!superUrg.length) {
    ...
    // URGENTE
    html += `<div class="card">
      <h2 style="color:#E67E22">&#9888;&#65039; Urgente — bajo minimo (${urgente.length})</h2>`;
    if (!urgente.length) {
    ...
    // SIN URGENCIA
    html += `<div class="card">
      <h2 style="color:#1D9E75">&#9989; Sin urgencia — stock suficiente (${ok.length})</h2>`;
    if (!ok.length) {
