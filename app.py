...
<div class="container">
  ...
  <div class="section active" id="tab-entrada">
    <div class="card">
      ...
      <input type="text" id="buscar-flor" placeholder="Escribe para buscar... ej: Rose Red" oninput="filtrarFlores()" autocomplete="off">
      <div id="lista-flores" style="display:none;max-height:200px;overflow-y:auto;border:1px solid #ddd;border-radius:8px;background:white;margin-top:4px;"></div>
      <div id="flor-seleccionada" style="display:none;background:#E0F5EC;border-radius:8px;padding:10px 12px;margin-top:6px;font-size:14px;font-weight:600;color:#0F6E56;display:flex;align-items:center;justify-content:space-between;">
        <span id="flor-seleccionada-nombre"></span>
        <span onclick="limpiarFlor()" style="cursor:pointer;color:#888;font-size:18px">✕</span>
      </div>
...
function filtrarFlores() {
  const q = document.getElementById("buscar-flor").value.trim().toLowerCase();
  const lista = document.getElementById("lista-flores");
  if (q.length < 1) { lista.style.display = "none"; return; }
  const matches = CATALOGO.filter(p => p.nombre.toLowerCase().includes(q) || p.categoria.toLowerCase().includes(q)).slice(0, 15);
  if (!matches.length) { lista.style.display = "none"; return; }
  lista.innerHTML = "";
  matches.forEach(p => {
    const div = document.createElement("div");
    div.style.cssText = "padding:10px 12px;cursor:pointer;border-bottom:1px solid #eee;font-size:14px;";
    div.innerHTML = `<span style="color:#888;font-size:11px">${p.categoria}</span><br>${p.nombre}`;
    div.onmousedown = () => elegirFlor(p.nombre);
    div.onmouseover = () => div.style.background = "#f0f7f4";
...
function elegirFlor(nombre) {
  florSeleccionada = nombre;
  document.getElementById("buscar-flor").value = "";
  document.getElementById("lista-flores").style.display = "none";
  const box = document.getElementById("flor-seleccionada");
  document.getElementById("flor-seleccionada-nombre").textContent = "✓ " + nombre;
  box.style.display = "flex";
}

function limpiarFlor() {
  florSeleccionada = "";
  document.getElementById("flor-seleccionada").style.display = "none";
  document.getElementById("buscar-flor").value = "";
}
...
async function registrar() {
  ...
  if (!flor) { showMsg("Busca y selecciona una flor", "err"); return; }
