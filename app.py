...
def init_db():
    ...
    # Tabla de motivos (por que se fue a nacional o se dio de baja)
    c.execute("""CREATE TABLE IF NOT EXISTS motivos (
        ...
        activo   BOOLEAN NOT NULL DEFAULT true,
        UNIQUE (aplica_a, codigo)
    )""")

    # Campos para estadistica y costeo. Si ya existen, no hace nada.
    c.execute("""ALTER TABLE movimientos
        ADD COLUMN IF NOT EXISTS motivo_id       INTEGER REFERENCES motivos(id),
        ADD COLUMN IF NOT EXISTS flor_id         INTEGER,
        ADD COLUMN IF NOT EXISTS costo_unitario  NUMERIC(12,4),
        ADD COLUMN IF NOT EXISTS costo_unidad    TEXT,
        ADD COLUMN IF NOT EXISTS tallos_por_ramo INTEGER,
        ADD COLUMN IF NOT EXISTS costo_fecha     DATE""")
...
<div class="tabs">
  ...
  <div class="tab" onclick="showTab('baja',this)">&#128683; Baja de Flor / Cambio</div>
...
<div class="container">
  ...
  <div class="section active" id="tab-entrada">
    <div class="card">
      ...
      <select id="e-proveedor" style="width:100%;padding:12px;border:1px solid #ddd;border-radius:8px;font-size:15px;background:#fff"><option value="">Cargando proveedores...</option></select>
  ...
  <div class="section" id="tab-baja">
    <div class="card">
      <h2>&#128683; Baja de Flor o Cambio en despacho</h2>
      <label>Tipo</label>
      <div class="tipo-selector">
        <div class="tipo-btn activo-rojo" id="btn-dano" onclick="setTipo('dano')">&#128683; Baja de Flor</div>
        <div class="tipo-btn" id="btn-cambio" onclick="setTipo('cambio')">&#8646; Cambio despacho</div>
      </div>
      <label>Proveedor</label><select id="b-proveedor" style="width:100%;padding:12px;border:1px solid #ddd;border-radius:8px;font-size:15px;background:#fff"><option value="">Cargando proveedores...</option></select>
      <label>Fecha</label><input type="date" id="b-fecha">
      <input type="hidden" id="b-usuario">
      <div id="sec-dano">
        <label>&#191;A donde va?</label>
        <div class="tipo-selector">
          <div class="tipo-btn activo-rojo" id="btn-nacional" onclick="setDestino('NACIONAL')">&#127796; Nacional</div>
          <div class="tipo-btn" id="btn-baja" onclick="setDestino('BAJA')">&#128465; Baja</div>
        </div>
        <div id="ayuda-destino" style="background:#FFF3E0;border-radius:8px;padding:10px;margin-top:8px;font-size:12px;color:#854F0B">
          <strong>Nacional:</strong> la flor tiene un defecto y no se puede exportar.
        </div>
        <label>Motivo</label>
        <select id="b-motivo" style="width:100%;padding:12px;border:1px solid #ddd;border-radius:8px;font-size:15px;background:#fff">
        ...
        <div class="row2">
          <div><label>Ramos</label><input type="number" id="b-ramos" min="0" value="0"></div>
...
async function cargarProveedores() {
  ...
  const html = (lista.length)
    ...
    : '<option value="">No hay proveedores. Agregalos desde el ERP.</option>';

  ["e-proveedor","b-proveedor"].forEach(function(id) {
...
// ── Motivos de nacional / baja ────────────────────────────────────────────
var MOTIVOS = {NACIONAL: [], BAJA: []};
var destinoActual = "NACIONAL";

async function cargarMotivos() {
  try {
    const res = await fetch("/motivos");
    MOTIVOS = await res.json();
  } catch(e) {
    MOTIVOS = {NACIONAL: [], BAJA: []};
  }
  pintarMotivos();
}

function pintarMotivos() {
  const sel = document.getElementById("b-motivo");
  if (!sel) return;
  const lista = MOTIVOS[destinoActual] || [];
  let html = '<option value="">-- Selecciona el motivo --</option>';
  let grupoActual = null;
  lista.forEach(function(m) {
    if (m.grupo !== grupoActual) {
      if (grupoActual !== null) html += '</optgroup>';
      html += '<optgroup label="' + m.grupo + '">';
      grupoActual = m.grupo;
    }
    html += '<option value="' + m.id + '">' + m.codigo + '</option>';
  });
  if (grupoActual !== null) html += '</optgroup>';
  sel.innerHTML = html;
}
...
function setDestino(d) {
  destinoActual = d;
  document.getElementById("btn-nacional").className = "tipo-btn" + (d==="NACIONAL" ? " activo-rojo" : "");
  document.getElementById("btn-baja").className = "tipo-btn" + (d==="BAJA" ? " activo-rojo" : "");
  document.getElementById("ayuda-destino").innerHTML = (d==="NACIONAL")
    ? "<strong>Nacional:</strong> la flor tiene un defecto y no se puede exportar."
    : "<strong>Baja:</strong> la flor esta buena pero no hay pedido para despacharla.";
  pintarMotivos();
}
...
async function registrarBaja() {
  ...
  if (tipoActual === "dano") {
    ...
    const motivo_id = document.getElementById("b-motivo").value;
    if (!proveedor) { showMsg("b-msg","Selecciona el proveedor","err"); return; }
    if (!flor) { showMsg("b-msg","Selecciona la flor","err"); return; }
    if (ramos<=0 && tallos<=0) { showMsg("b-msg","Ingresa cantidad","err"); return; }
    if (!motivo_id) { showMsg("b-msg","Selecciona el motivo","err"); return; }
    const res = await fetch("/baja", {
      method:"POST", headers:{"Content-Type":"application/json"},
      body: JSON.stringify({tipo:destinoActual, motivo_id, flor, ramos, tallos, proveedor, fecha, usuario, notas})
    });
    const data = await res.json();
    if (data.ok) {
      showMsg("b-msg","✓ "+(destinoActual==="NACIONAL"?"Nacional":"Baja")+" registrada: "+flor,"ok");
      limpiarTag("b");
...
function bajarCSV(nombre, filas) {
  ...
  const BOM   = String.fromCharCode(65279);  // para que Excel respete tildes
  const peligroso = new RegExp('[";' + SALTO + ']');
  const csv = BOM + filas.map(function(f) {
    return f.map(function(c) {
      const v = String(c === null || c === undefined ? "" : c);
      return peligroso.test(v) ? '"' + v.split('"').join('""') + '"' : v;
    }).join(";");
  }).join(SALTO);
...
async function exportarStock() {
  try {
    ...
    const filas = [["Flor","Categoria","Ramos","Tallos sueltos"]];
    data.forEach(f => filas.push([f.nombre, f.categoria || "",
                                  f.stock_ramos || 0, f.stock_tallos || 0]));
    bajarCSV("stock_" + hoyTexto() + ".csv", filas);
  } catch(e) { alert("No se pudo descargar el stock."); }
}

async function exportarDatos() {
  try {
    const data = await (await fetch("/stock")).json();
    const filas = [["Prioridad","Flor","Categoria","Ramos","Tallos sueltos"]];
    const grupo = f => enNegativo(f) ? "SUPER URGENTE"
                     : (f.stock_ramos < 50 ? "URGENTE" : "SIN URGENCIA");
    const orden = {"SUPER URGENTE":0, "URGENTE":1, "SIN URGENCIA":2};
    data.slice()
        .sort((a,b) => orden[grupo(a)] - orden[grupo(b)] || a.stock_ramos - b.stock_ramos)
        .forEach(f => filas.push([grupo(f), f.nombre, f.categoria || "",
                                  f.stock_ramos || 0, f.stock_tallos || 0]));
    bajarCSV("compras_" + hoyTexto() + ".csv", filas);
  } catch(e) { alert("No se pudieron descargar los datos."); }
}

// Muestra el faltante o el stock EXACTO, sin perder los tallos sueltos.
function fmtStock(f) {
  const r = Math.abs(f.stock_ramos || 0);
  const t = Math.abs(f.stock_tallos || 0);
  if (!r && !t) return "0 ramos";
  let s = "";
  if (r) s += r + " ramos";
  if (t) s += (s ? " + " : "") + t + " tallos";
  return s;
...
function enNegativo(f) {
  return (f.stock_ramos || 0) < 0 || (f.stock_tallos || 0) < 0;
}

function fmtTotal(ramos, tallos) {
  let s = "";
  if (ramos) s += ramos + " ramos";
  if (tallos) s += (s ? " + " : "") + tallos + " tallos";
  return s || "0 ramos";
}
...
async function cargarDatos() {
  ...
  try {
    ...
    const superUrg = data.filter(f => enNegativo(f)).sort((a,b) => a.stock_ramos - b.stock_ramos);
    const urgente  = data.filter(f => !enNegativo(f) && f.stock_ramos < 50).sort((a,b) => a.stock_ramos - b.stock_ramos);
    const ok       = data.filter(f => !enNegativo(f) && f.stock_ramos >= 50).sort((a,b) => b.stock_ramos - a.stock_ramos);

    // Totales por grupo, contando ramos Y tallos sueltos
    const ramosSuper  = superUrg.reduce((s,f) => s + Math.abs(f.stock_ramos || 0), 0);
    const tallosSuper = superUrg.reduce((s,f) => s + Math.abs(f.stock_tallos || 0), 0);
    const ramosUrg    = urgente.reduce((s,f) => s + (f.stock_ramos || 0), 0);
    const tallosUrg   = urgente.reduce((s,f) => s + (f.stock_tallos || 0), 0);
    const ramosOk     = ok.reduce((s,f) => s + (f.stock_ramos || 0), 0);
    const tallosOk    = ok.reduce((s,f) => s + (f.stock_tallos || 0), 0);

    const totSuper = fmtTotal(ramosSuper, tallosSuper);
    const totUrg   = fmtTotal(ramosUrg, tallosUrg);
    const totOk    = fmtTotal(ramosOk, tallosOk);
    ...
    html += `<div class="card" style="padding:0;overflow:hidden">
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;text-align:center">
        <div style="padding:14px 6px;background:#FDECEA">
          ...
          <div style="font-size:13px;color:#A32D2D;font-weight:700;margin-top:4px">${totSuper}</div>
        </div>
        <div style="padding:14px 6px;background:#FFF3E0">
          ...
          <div style="font-size:13px;color:#854F0B;font-weight:700;margin-top:4px">${totUrg}</div>
        </div>
        <div style="padding:14px 6px;background:#E0F5EC">
          ...
          <div style="font-size:13px;color:#0F6E56;font-weight:700;margin-top:4px">${totOk}</div>
        </div>
      </div>
    </div>`;
    ...
    // SUPER URGENTE
    html += `<div class="card">
      <h2 style="color:#C0392B">&#128680; Super urgente — conseguir ya (${superUrg.length} flores · ${totSuper})</h2>`;
    if (!superUrg.length) {
      html += `<p style="font-size:13px;color:#0F6E56;padding:6px 0">Ninguna flor en negativo.</p>`;
    } else {
      html += `<p style="font-size:11px;color:#888;margin-bottom:8px">Stock en negativo. Es lo que falta EXACTO: si dice ramos y tallos, hay que conseguir los dos.</p>`;
      ...
      superUrg.forEach(f => {
        html += `<tr><td style="color:#C0392B;font-weight:600">${f.nombre}</td>
                 <td style="text-align:center;color:#C0392B;font-weight:700">${fmtStock(f)}</td></tr>`;
      });
      html += `</table>`;
    }
    html += `</div>`;
    ...
    // URGENTE
    html += `<div class="card">
      <h2 style="color:#E67E22">&#9888;&#65039; Urgente — bajo minimo (${urgente.length} flores · ${totUrg})</h2>`;
    if (!urgente.length) {
      ...
      urgente.forEach(f => {
        html += `<tr><td style="color:#E67E22;font-weight:500">${f.nombre}</td>
                 <td style="text-align:center;color:#E67E22;font-weight:600">${fmtStock(f)}</td></tr>`;
      });
      html += `</table>`;
    }
    html += `</div>`;
    ...
    // SIN URGENCIA
    html += `<div class="card">
      <h2 style="color:#1D9E75">&#9989; Sin urgencia — stock suficiente (${ok.length} flores · ${totOk})</h2>`;
    if (!ok.length) {
      ...
      ok.forEach(f => {
        html += `<tr><td>${f.nombre}</td>
                 <td style="text-align:center;color:#0F6E56;font-weight:600">${fmtStock(f)}</td></tr>`;
      });
      html += `</table>`;
    }
    html += `</div>`;
...
async function cargarHistorial() {
  ...
  try {
    ...
    data.forEach(m => {
      ...
      else if (m.tipo.includes("BAJA") || m.tipo.includes("NACIONAL")) { clase="mov-salida"; icono="&#128683;"; }
...
async function verFicha(nombre) {
  ...
  if (!data.movimientos.length) {
    ...
    data.movimientos.forEach(m => {
      ...
      if (m.tipo.includes("BAJA") || m.tipo.includes("SALIDA") || m.tipo.includes("NACIONAL")) { color="#C0392B"; ico="&#128683;"; }
...
def entrada():
    ...
    try:
        ...
        if not _proveedor_valido(c, proveedor):
            conn.close()
            return jsonify({"ok": False,
...
def _proveedor_valido(cursor, nombre):
    """
    El proveedor tiene que estar en la lista. El desplegable ya lo limita,
    pero la validacion de verdad va aqui: si no, cualquiera podria mandar
    un nombre inventado y volveriamos al texto libre.
    """
    if not nombre:
        return False
    cursor.execute("""SELECT 1 FROM proveedores
...
def proveedores():
    """Lista de proveedores para el desplegable de Entrada y Baja."""
    try:
        conn = get_conn()
        c = conn.cursor()
        c.execute("""SELECT nombre FROM proveedores
                     WHERE activo = true ORDER BY nombre""")
        lista = [f[0] for f in c.fetchall()]
        conn.close()
...
def motivos():
    """
    Lista de motivos separada en NACIONAL y BAJA, ya ordenada por grupo.
    La app la carga una sola vez al abrir sesion.
    """
    try:
        conn = get_conn()
        c = conn.cursor()
        c.execute("""SELECT id, aplica_a, codigo, grupo
                     FROM motivos WHERE activo = true
                     ORDER BY aplica_a, orden""")
        salida = {"NACIONAL": [], "BAJA": []}
        for mid, aplica, codigo, grupo in c.fetchall():
            if aplica in salida:
                salida[aplica].append({"id": mid, "codigo": codigo, "grupo": grupo})
        conn.close()
        return jsonify(salida)
    except Exception:
        return jsonify({"NACIONAL": [], "BAJA": []})


@app.route("/baja", methods=["POST"])
def baja():
    """
    Salida de inventario que NO es despacho de exportacion.

      NACIONAL -> la flor tiene un defecto y se va al mercado nacional
      BAJA     -> la flor esta buena pero no hay pedido

    En los dos casos el motivo es obligatorio: es lo que despues permite
    saber cuanto se perdio y por que.
    """
    data = request.json
    ...
    if tipo in ("BAJA_DANO", "BAJA_DAÑO", ""):   # compatibilidad con la version vieja
        tipo = "BAJA"
    if tipo not in ("NACIONAL", "BAJA"):
        return jsonify({"ok": False, "error": "Destino invalido"})

    motivo_id = data.get("motivo_id")
    try:
        motivo_id = int(motivo_id) if motivo_id not in (None, "") else None
    ...
    if motivo_id is None:
        return jsonify({"ok": False, "error": "Selecciona el motivo"})
    ...
    try:
        ...
        # El motivo tiene que existir y corresponder al destino escogido
        c.execute("SELECT aplica_a FROM motivos WHERE id=%s AND activo=true",
                  (motivo_id,))
        row = c.fetchone()
        if not row or row[0] != tipo:
            conn.close()
            return jsonify({"ok": False,
                            "error": "El motivo no corresponde al destino"})
        ...
        c.execute(
            """INSERT INTO movimientos
               (fecha, flor_nombre, flor_id, tipo, cantidad_ramos, cantidad_tallos,
                origen, proveedor, fecha_remision, usuario, notas, motivo_id)
               VALUES (%s, %s,
                       (SELECT id FROM flores WHERE LOWER(nombre)=LOWER(%s)),
                       %s, %s, %s, %s, %s, %s, %s, %s, %s)""",
            (ahora().strftime("%Y-%m-%d %H:%M"), flor, flor, tipo, ramos, tallos,
             tipo, proveedor, fecha or None, usuario, notas, motivo_id)
        )
        normalizar_stock(c, flor)
        conn.commit()
        conn.close()
...
def eliminar_movimiento():
    ...
    try:
        ...
        elif tipo in ("NACIONAL", "BAJA", "BAJA_DANO", "BAJA_DAÑO", "CAMBIO_SALIDA"):
            c.execute("""UPDATE flores SET stock_ramos=stock_ramos+%s,
                         stock_tallos=stock_tallos+%s
                         WHERE LOWER(nombre)=LOWER(%s)""", (ramos, tallos, flor))
