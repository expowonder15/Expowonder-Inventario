...
function renderStock(data) {
  ...
  data.forEach(f => {
    html += `<tr><td>${f.nombre}</td><td style="color:#888;font-size:11px">${f.categoria}</td><td style="text-align:center">${f.stock_ramos}</td><td style="text-align:center">${f.stock_tallos}</td></tr>`;
  });
  html += "</table>";
  div.innerHTML = html;
}
...
def entrada():
    ...
    try:
        ...
        for item in items:
            ...
            if not c.fetchone():
                c.execute("INSERT INTO flores (nombre, categoria) VALUES (%s, %s)", (flor, "Sin categoria"))
            c.execute(
...
def cambio():
    ...
    try:
        ...
        if not original_no_estaba:
            # La original sí estaba — se devuelve al stock
            c.execute(
                "UPDATE flores SET stock_ramos=stock_ramos+%s, stock_tallos=stock_tallos+%s WHERE LOWER(nombre)=LOWER(%s)",
