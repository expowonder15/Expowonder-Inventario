...
<div class="header">
  ...
  <p>Control de inventario de flores <span style="opacity:.6">v3.0</span></p>
</div>
<div class="tabs">
...
<div class="container">
  ...
  <div class="section active" id="tab-entrada">
    <div class="card">
      ...
      <label>Quien registra <span style="color:#C0392B">*</span></label>
      <select id="e-usuario"><option value="">-- Selecciona tu nombre --</option></select>
  ...
  <div class="section" id="tab-baja">
    <div class="card">
      ...
      <label>Quien registra <span style="color:#C0392B">*</span></label>
      <select id="b-usuario"><option value="">-- Selecciona tu nombre --</option></select>
      <div id="sec-dano">
  ...
  <div class="section" id="tab-ajuste">
    <div class="card">
      ...
      <label>Quien hace el ajuste <span style="color:#C0392B">*</span></label>
      <select id="aj-usuario"><option value="">-- Selecciona tu nombre --</option></select>
  ...
  <div class="section" id="tab-historial">
    <div class="card">
      ...
      <p style="font-size:12px;color:#888;margin-bottom:12px">Ultimos 50 movimientos. Toca Eliminar para corregir un error.</p>
...
// ── Exportar a Excel (formato CSV con BOM, abre directo en Excel) ────────
...
async function exportarStock() {
  ...
  // Respetar el filtro que este aplicado en pantalla
  const q = (document.getElementById("s-filtro").value || "").toLowerCase();
  if (q) data = data.filter(f => f.nombre.toLowerCase().includes(q) || f.categoria.toLowerCase().includes(q));

  const filas = [["STOCK ACTUAL - EXPOWONDER"], ["Generado", new Date().toLocaleString()], [],
                 ["Flor","Categoria","Ramos","Tallos","Estado"]];
  data.forEach(f => {
    const estado = f.stock_ramos < 0 ? "SUPER URGENTE"
                 : (f.stock_ramos < 50 ? "URGENTE" : "OK");
    filas.push([f.nombre, f.categoria, f.stock_ramos, f.stock_tallos, estado]);
  });
  filas.push([]);
  filas.push(["TOTAL FLORES", data.length, "", "", ""]);
  filas.push(["TOTAL RAMOS", data.reduce((s,f) => s + f.stock_ramos, 0), "", "", ""]);
  descargarCSV(`stock_${fechaHoy()}.csv`, filas);
}
...
async function exportarDatos() {
  ...
  const rSuper = superUrg.reduce((s,f) => s + Math.abs(f.stock_ramos), 0);
  const rUrg   = urgente.reduce((s,f) => s + f.stock_ramos, 0);
  const rOk    = ok.reduce((s,f) => s + f.stock_ramos, 0);

  const filas = [["DATOS - SEMAFORO DE INVENTARIO"], ["Generado", new Date().toLocaleString()], [],
                 ["RESUMEN","Flores","Ramos"],
                 ["Super urgente", superUrg.length, rSuper],
                 ["Urgente",       urgente.length,  rUrg],
                 ["Sin urgencia",  ok.length,       rOk],
                 []];

  filas.push([`SUPER URGENTE - conseguir ya (${superUrg.length} flores | ${rSuper} ramos)`]);
  filas.push(["Flor","Categoria","Faltan (ramos)"]);
  superUrg.forEach(f => filas.push([f.nombre, f.categoria, Math.abs(f.stock_ramos)]));
  filas.push([]);

  filas.push([`URGENTE - bajo minimo (${urgente.length} flores | ${rUrg} ramos)`]);
  filas.push(["Flor","Categoria","Stock (ramos)"]);
  urgente.forEach(f => filas.push([f.nombre, f.categoria, f.stock_ramos]));
  filas.push([]);

  filas.push([`SIN URGENCIA - stock suficiente (${ok.length} flores | ${rOk} ramos)`]);
  filas.push(["Flor","Categoria","Stock (ramos)"]);
  ok.forEach(f => filas.push([f.nombre, f.categoria, f.stock_ramos]));

  descargarCSV(`datos_${fechaHoy()}.csv`, filas);
}
...
async function registrarAjuste() {
  const usuario = document.getElementById("aj-usuario").value;
...
def cambio():
    ...
    try:
        ...
        c.execute(
            "INSERT INTO movimientos (fecha, flor_nombre, tipo, cantidad_ramos, cantidad_tallos, origen, proveedor, fecha_remision, usuario, notas) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
            (datetime.now().strftime("%Y-%m-%d %H:%M"), flor_orig, "CAMBIO_DEVOLUCION", ramos, tallos, "CAMBIO", proveedor, fecha, usuario, nota_orig)
        )
        c.execute(
            "UPDATE flores SET stock_ramos=stock_ramos-%s, stock_tallos=stock_tallos-%s WHERE LOWER(nombre)=LOWER(%s)",
...
def eliminar_movimiento():
    ...
    try:
        conn = get_conn()
        c = conn.cursor()
        c.execute("SELECT flor_nombre, tipo, cantidad_ramos, cantidad_tallos FROM movimientos WHERE id=%s", (mov_id,))
        mov = c.fetchone()
        if mov:
            flor, tipo, ramos, tallos = mov
            if tipo == "ENTRADA":
                c.execute("UPDATE flores SET stock_ramos=stock_ramos-%s, stock_tallos=stock_tallos-%s WHERE LOWER(nombre)=LOWER(%s)", (ramos, tallos, flor))
...
def ajuste():
    ...
    try:
        ...
        c.execute("""INSERT INTO movimientos
                  ...
                  (datetime.now().strftime("%Y-%m-%d %H:%M"), flor, dif_r, dif_t,
                   usuario, nota))
        conn.commit()
        conn.close()
