...
<div class="header">
  ...
  <p>Control de inventario de flores <span style="opacity:.6">v3.3</span></p>
</div>
<div class="tabs">
...
function eliminarMov(id, nombre, tipo, cantidad) {
  // Ventana para confirmar: quien elimina y por que
  const opciones = PERSONAL.map(p =>
    `<option value="${p.nombre}">${p.nombre}</option>`).join("");
    ...
    <div id="modal-elim" style="position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:600;
      ...
      <div style="background:white;border-radius:12px;max-width:420px;width:100%;padding:18px"
        ...
        <label style="font-size:12px;color:#666;display:block;margin-bottom:3px">
          Quien lo elimina <span style="color:#C0392B">*</span></label>
        <select id="elim-usuario" style="width:100%;padding:10px 12px;border:1px solid #ddd;
                border-radius:8px;font-size:15px;background:#fafafa;margin-bottom:10px">
...
async function confirmarEliminar(id) {
  const usuario = document.getElementById("elim-usuario").value;
  const motivo  = document.getElementById("elim-motivo").value.trim();
  const msg     = document.getElementById("elim-msg");

  if (!usuario) { msg.innerHTML = "<span style='color:#C0392B'>Selecciona tu nombre</span>"; return; }
...
def eliminar_movimiento():
    ...
    try:
        ...
        c.execute("""INSERT INTO movimientos
                  ...
                  (datetime.now().strftime("%Y-%m-%d %H:%M"), flor,
