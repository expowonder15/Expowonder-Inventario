...
async function cargarAlertas() {
  const res = await fetch("/stock");
  const data = await res.json();
  const negativos = data.filter(f => f.stock_ramos < 0);
  const bajos = data.filter(f => f.stock_ramos >= 0 && f.stock_ramos < f.alerta_minimo && (f.stock_ramos > 0 || f.stock_tallos > 0));
  const box = document.getElementById("alertas-box");
  let html = "";
  if (negativos.length) {
    html += `<div class="alerta-box" style="background:#FCEBEB"><h3 style="color:#A32D2D">&#128680; ${negativos.length} flores con stock negativo</h3>`;
    negativos.forEach(a => { html += `<div class="alerta-item" style="color:#A32D2D;border-color:#F5C6C6">${a.nombre} — ${a.stock_ramos} ramos</div>`; });
    html += "</div>";
  }
  if (bajos.length) {
    html += `<div class="alerta-box"><h3>&#9888;&#65039; ${bajos.length} flores bajo minimo</h3>`;
    bajos.forEach(a => { html += `<div class="alerta-item">${a.nombre} — ${a.stock_ramos} ramos</div>`; });
    html += "</div>";
  }
  box.innerHTML = html;
}
